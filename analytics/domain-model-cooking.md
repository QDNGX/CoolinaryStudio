...
`NO_SHOW` проставляет Chef или Admin через свой интерфейс (теперь он у нас в скоупе).
...
- **D-02 — лимит поздних отмен.** `lateCancelCount` растёт на 1 при `isLateCancellation` или
  `NO_SHOW`. При `lateCancelCount >= 3` новые брони блокируются.
  *Предположение: счётчик пожизненный — уточнить, нужен ли сброс.*
...
- **D-07 — уведомления по email.** Отмена студией и (если решите добавить) напоминания уходят на
  `User.email` — он есть всегда, так как то же поле используется для входа. Ошибка отправки письма
  не должна блокировать бронь/отмену — это best-effort side-effect, а не часть транзакции.
- **D-08 — заведение аккаунтов Chef.** Предположение: шеф не регистрируется сам — Admin заводит
...
- Сброс `lateCancelCount` — разовый или периодический?
- Кто утверждает `NO_SHOW` — только Chef, только Admin, или оба?
