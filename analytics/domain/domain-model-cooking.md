...
Код подтверждения (шесть цифр) — не доменная сущность в смысле `User`/`Booking`: короткоживущая
запись с TTL (таблица с очисткой по расписанию или кэш), общая для всех трёх ролей. Параметры
зафиксированы (Р-09): срок действия 10 минут, до 5 попыток ввода, повторная отправка — не чаще раза в 60 секунд.
...
|---|---|---|
| `user` | `User` (ref) | |
| `photo` | `String` | редактирует Admin в разделе «Шефы» (Р-05) |
| `bio` | `String` | редактирует Admin в разделе «Шефы» (Р-05) |
...
| `requiresComplexEquipment` | `boolean` | если true — лимит класса 8 человек вместо 12; смена признака действует только на новые слоты, уже созданные сохраняют вместимость (Р-04) |
| `dishes` | `List<String>` | |
| `photos` | `List<String>` | |
...
| `status` | `enum` (`SCHEDULED`, `CANCELLED_BY_STUDIO`, `COMPLETED`) | `CANCELLED_BY_STUDIO` ставит Admin; `COMPLETED` — автоматически по окончании (D-09) |
...
ведёт класс и видит фактическую явку; Admin статус не проставляет, но может снять ошибочную отметку (D-11).
...
- **D-09 — автозавершение слота.** По наступлении `startAt + durationMinutes` слот автоматически
  переходит в `COMPLETED`; связанные брони в `CONFIRMED` — тоже в `COMPLETED` (кроме уже
  отмеченных `NO_SHOW`). Ручного действия «завершить» у Admin нет (Р-10).
- **D-10 — запрет пересечения слотов шефа.** Нельзя создать слот, время которого пересекается
  с другим слотом того же шефа в статусе `SCHEDULED`; при попытке — явный отказ с указанием
  конфликтующего слота (Р-08).
- **D-11 — снятие ошибочной неявки.** Admin может снять отметку `NO_SHOW` (статус брони →
  `COMPLETED`); `lateCancelCount` уменьшается на 1, и если условие блокировки перестаёт
  выполняться — `blockedUntil` снимается (Р-02). У Chef обратного действия нет.
...
## Вход и первая регистрация клиента (решения Р-01, Р-12)

Клиент не проходит отдельную регистрацию: если `User` с введённым email не существует, при первом
успешном подтверждении кода система молчаливо создаёт `User(role=CLIENT)` + `ClientProfile`.
Сразу после первого входа клиенту задаётся один вопрос — имя («Как к вам обращаться?»), которое
заполняет `User.name`. Для ролей `CHEF`/`ADMIN` самосоздания нет — их аккаунты заводятся заранее
(D-08); при входе с неизвестным email для этих ролей — подсказка обратиться к студии. Это уточняет
предусловие UC-05: существование `User` обязательно только для Chef/Admin.

Полный перечень решений, принятых при постановке экранов на дизайн, —
[design-decisions-cooking.md](../design-brief/design-decisions-cooking.md) (Р-01…Р-16).
